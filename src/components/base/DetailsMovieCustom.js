 ...
 makeMoreRequest = async  () => {
   ...
   //setTimeout( async () => {
    ...
    this.setState({currentSimilarPage : this.state.currentSimilarPage +1},  async () => {
      ...
      await axios.get(`${Constant.TMDB_URL}/movie/${this.props.details.id}/similar?api_key=${Constant.TMDB_API_KEY}&language=en-US&page=${this.state.currentSimilarPage}`).then(
          (res) => {
