   ...
   let settings = await getSettings(LANGUAGE_KEY);
    this.setState({currentSimilarPage : this.state.currentSimilarPage +1},  async () => {
      ...
      if (this.state.currentSimilarPage <= this.props.similarmovies.total_pages) {
        ...
        await axios.get(`${Constant.TMDB_URL}/movie/${this.props.details.id}/similar?api_key=${Constant.TMDB_API_KEY}&language=${settings}&page=${this.state.currentSimilarPage}`).then(
          (res) => {
    ...
    return (
      this.state.isLoading ?
        ...
        <View style={{ flex: 1, marginBottom: 20 }}>
          <Text style={{ textAlign: 'left', justifyContent: 'center', alignItems: 'flex-start', paddingVertical: 10, fontSize: 20 ,color :'gray' }}> Similar Movie</Text>
          <FlatList
